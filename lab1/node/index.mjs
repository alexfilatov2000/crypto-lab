...
function getDecryptedTexts(text) {
    ...
    return keys.map(
        (key) => [
            ...
            [...text].reduce(
                (accum, [char]) => {
                    return accum + String.fromCharCode(char.charCodeAt() ^ key)
                },
                ''
            )
        ]
...
const decryptedTexts = getDecryptedTexts(Buffer.from(cipheredText, 'hex').toString('utf-8'));
